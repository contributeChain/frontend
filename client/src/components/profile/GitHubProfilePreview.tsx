  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="px-6 -mt-16 relative">
        <img
          src={extendedGithubUser.avatar_url}
          alt={extendedGithubUser.login}
      ...
      <CardHeader className="pt-2">
        <CardTitle className="text-2xl flex items-center gap-2">
          {extendedGithubUser.name || extendedGithubUser.login}
          {extendedGithubUser.email && (
        ...
        <CardDescription className="flex items-center gap-2">
          ...
          @{extendedGithubUser.login}
        </CardDescription>
      </CardHeader>
      
      <CardContent className="space-y-4">
        {extendedGithubUser.bio && (
          <p className="text-sm">{extendedGithubUser.bio}</p>
        )}
        
        <div className="grid grid-cols-2 gap-4 text-sm">
          {extendedGithubUser.company && (
            <div className="flex items-center gap-2">
              ...
              <span>{extendedGithubUser.company}</span>
            </div>
          )}
          
          {extendedGithubUser.location && (
            <div className="flex items-center gap-2">
              ...
              <span>{extendedGithubUser.location}</span>
            </div>
          )}
          ...
          {extendedGithubUser.email && (
            <div className="flex items-center gap-2">
              ...
              <span>{extendedGithubUser.email}</span>
            </div>
          )}
          
          {extendedGithubUser.blog && (
            <div className="flex items-center gap-2">
              ...
              <a href={/^https?:\/\//.test(extendedGithubUser.blog) ? extendedGithubUser.blog : `https://${extendedGithubUser.blog}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline truncate">
                {extendedGithubUser.blog}
              </a>
            </div>
          )}
        ...
        <div className="grid grid-cols-3 gap-4 mt-4 border-t border-b py-4">
          ...
          <div className="text-center border-x">
            <div className="text-2xl font-bold">{extendedGithubUser.followers || 0}</div>
            <div className="text-xs text-muted-foreground">Followers</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold">{extendedGithubUser.following || 0}</div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">Connected to wallet {walletAddress.substring(0, 6)}...{walletAddress.substring(walletAddress.length - 4)}</span>
        </div>
      </CardContent>
